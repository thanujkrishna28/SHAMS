...
export default Mess; onClick = {() => onAdd(type)}
className = "w-10 h-10 bg-slate-50 text-slate-400 hover:text-slate-900 hover:bg-slate-100 rounded-xl transition-all flex items-center justify-center active:scale-90"
    >
    <Plus size={18} />
                </button >
            </div >
    <div className="space-y-2.5">
        {items?.length > 0 ? (
            items.map((item: string, i: number) => (
                <div key={i} className="flex items-center justify-between bg-slate-50 group/item hover:bg-slate-900 hover:text-white px-5 py-4 rounded-xl text-xs font-bold text-slate-700 transition-all">
                    <span className="uppercase tracking-tight">{item}</span>
                    <button
                        onClick={() => onRemove(type, i)}
                        className="text-slate-300 hover:text-rose-400 transition-colors opacity-0 group-hover/item:opacity-100"
                    >
                        <X size={16} />
                    </button>
                </div>
            ...
            <div className="py-8 text-center border-2 border-dashed border-slate-100 rounded-[1.5rem]">
                <p className="text-[9px] font-black text-slate-300 uppercase tracking-[0.2em]">Zero Items Defined</p>
            </div>
        )}
    </div>
        </div >
...
const MetricTile = ({ label, value, icon, color }: any) => {
    const colors: any = {
        ...
        rose: 'bg-rose-500 text-white shadow-rose-900/20',
        blue: 'bg-blue-600 text-white shadow-blue-900/20',
    };

    return (
        <div className="bg-white/10 backdrop-blur-xl border border-white/10 p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] group transition-all hover:bg-white/20">
            <div className="flex items-center justify-between mb-2 md:mb-4">
                <div className={`w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl ${colors[color]} flex items-center justify-center shadow-lg transition-transform group-hover:scale-110 duration-500`}>
                    {icon}
                </div>
                <span className="text-xl md:text-3xl font-black text-white tracking-tighter leading-none">{value}</span>
            </div>
            <p className="text-[8px] md:text-[10px] font-black text-white/50 uppercase tracking-[0.2em]">{label}</p>
        </div>
    );
};
